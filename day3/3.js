const getVisitedBlocks = (startLocation, instruction) => {
	const direction = instruction.substring(0, 1);
	const length = instruction.substring(1);
	let adder = [0, 0];
	switch (direction) {
		case "L":
			adder = [-1, 0];
	...
	for (let i = 0; i < length; i++) {
		...
		visitedBlocks.push([...currentLocation]);
	}
	return visitedBlocks;
};

const getBlocksFromInstructions = instructionList => {
	let visitedBlocks = [];
	let currentLocation = [0, 0];
	for (let i in instructionList) {
		visitedBlocks = visitedBlocks.concat(
			getVisitedBlocks(currentLocation, instructionList[i])
		);
		currentLocation = [...visitedBlocks[visitedBlocks.length - 1]];
	}
...
const findCrossings = (oldBlocks, newBlocks) => {
	let crossings = [];
	...
	for (let n = 0; n < newBlocks.length; n++) {
		for (let o = 0; o < oldBlocks.length; o++) {
			if (
				oldBlocks[o][0] == newBlocks[n][0] &&
				oldBlocks[o][1] == newBlocks[n][1]
			) {
				crossings.push(newBlocks[n]);
				return crossings;
			}
		}
		console.log(n, newBlocks.length, n / newBlocks.length, oldBlocks.length);
	}
	return crossings;
};

const getManhattanDistance = block => Math.abs(block[0]) + Math.abs(block[1]);
...
let allVisitedBlocks = [];
let allCrossings = [];

for (let i in instructions) {
	const instruction = instructions[i];
	console.log("adding blocks");
	const newBlocks = getBlocksFromInstructions(parseInstructions(instruction));
	console.log("sorting");
	newBlocks.sort((a, b) =>
		getManhattanDistance(a) < getManhattanDistance(b) ? -1 : 1
	);
	console.log("finding crossings");
	const newCrossings = findCrossings(allVisitedBlocks, newBlocks);
	console.log("found crossings");
	allCrossings = allCrossings.concat(newCrossings);
	allVisitedBlocks = allVisitedBlocks.concat(newBlocks);
	console.log("sorting");
	allVisitedBlocks.sort((a, b) =>
		getManhattanDistance(a) < getManhattanDistance(b) ? -1 : 1
	);
	console.log("loop end");
}
console.log("sorting for manhattan distance");

allCrossings.sort((a, b) =>
	getManhattanDistance(a) < getManhattanDistance(b) ? -1 : 1
);

console.log(allVisitedBlocks);
console.log(allCrossings);

console.log("closest crossing ", getManhattanDistance(allCrossings[0]));
